	...
	var toolbar=function(){
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			img1=img.replace("./", "/");
	                ...
	                h += "<td class='"+d.id+"_html_button' valign='middle' align='center' onclick='"+cmd+"' title='"+toolbar[i].tooltip+"'><image src='"+img1+"'></td>";
			    }
	            // END: Custom button
				else if(toolbar[i]==="separator"){
d1=d.idir.replace("./", "/");
h += "<td valign='middle' align='center'><image src='"+d1+"separator.gif' style='margin-right:1px;margin-left:3px;height:13px;'></td>";}
...
img1=img.replace("./", "/");
h += "<td class='"+d.id+"_html_button' valign='middle' align='center' onclick='global_hb[\""+d.id+"\"].cmd(\""+toolbar[i]+"\")' title='"+cmds[toolbar[i]]+"'><image src='"+img1+"'></td>";}
		    }
			h += "</table></td></tr>";
		}
		return h;
